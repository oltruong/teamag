...
public class ParameterController {
	...
	this.administratorEmailParameter = this.parametersEJB
		.getAdministratorEmailParameter();
    }

    public String doUpdateParameters() {
	this.parametersEJB.saveAndReloadParameters(this.smtpHostParameter,
		this.administratorEmailParameter);

	initList();

	FacesMessage msg = null;
	msg = new FacesMessage(FacesMessage.SEVERITY_INFO,
		"Mise à jour effectuée", "Paramètres mis à jour");
	...
	properties.setProperty("mail.smtp.host",
		this.smtpHostParameter.getValue());
	...
	try {
	    ...
	    message.setFrom(new InternetAddress("TEAMAG<"
	    ...
	    FacesMessage msg = null;
	    msg = new FacesMessage(FacesMessage.SEVERITY_INFO,
		    "Message envoyé", "Envoyé à "
			    + this.administratorEmailParameter.getValue());
	    FacesContext.getCurrentInstance().addMessage(null, msg);
	} catch (MessagingException messagingException) {
	    this.logger.severe("Error when sending message ["
